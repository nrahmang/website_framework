...
class Route
{
            ...
            foreach( self::$routes[$_SERVER["REQUEST_METHOD"]] as $page )
            {
                ...
                if ($a) {
                    ...
                    if( count($act) == 2 ) {
                        ...
                        $methodName = $act[1];
                        if( file_exists('framework/controllers/'.$controllerName.'.php')  ){
                            include_once('framework/controllers/'.$controllerName.'.php');
    ...
    public static function load(){
        foreach (scandir('framework/routes') as $f) {
            if( $f != '.' && $f != '..' ) {
                if (is_dir('framework/routes/' . $f)) {
                    foreach (scandir('framework/routes/'.$f) as $f2) {
                        if (substr($f2, -3, 3) == 'php') {
                            include_once('framework/routes/' . $f.'/'.$f2);
                        }
                    }
                } else {
                    if (substr($f, -3, 3) == 'php') {
                        include_once('framework/routes/'. $f);
                    }
                }
            }
        }
